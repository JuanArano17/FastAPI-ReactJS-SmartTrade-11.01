...
from schemas.seller_product import SellerProduct, SellerProductCreate, SellerProductUpdate
...
router = APIRouter(prefix="/product/{product_id}/seller_products", tags=["seller-products"])
...
async def update_seller_product(*, product_id=int, seller_product_id: int, seller_product: SellerProductUpdate):
    """
    Update a seller product.
    """
    existing_seller_product =  seller_product_service.get_by_id(seller_product_id=seller_product_id)
    
    if existing_seller_product is None or existing_seller_product.id_product != int(product_id):
        raise HTTPException(status_code=404, detail="Seller product not found")
    
    return seller_product_service.update(seller_product_id=seller_product_id, new_data=seller_product)

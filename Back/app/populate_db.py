...
for i in range(num_buyers):
    ...
    dni_letter = random.choice(string.ascii_uppercase)  # Generate a random uppercase letter
...
for i in range(num_cards):
    ...
    card_number = faker.credit_card_number(card_type=None)  # Generate a random card number
    card_name = faker.name()  # Generate a random cardholder name
    card_exp_date = faker.date_between(start_date=datetime.now(), end_date='+10y')  # Generate a random expiration date within the next 10 years
    card_security_num = faker.credit_card_security_code(card_type=None)  # Generate a random security number (CVV)
